# running example: 
# python2.7 follow-youtube-rec-firefox.py  --query="DACA" --searches=1 --branch=1 --depth=21 --name="test" --order=0 --security=0
...
def compare_keywords(driver, query, search_results, branching, depth, name, gl, language, recent, loopok, trending, sequence):
    ...
    file_name = 'results/' + name2 + '-' + date + '-' + order + '.json'
...
def main():
    ...
    parser.add_argument('--security', default=0, type=int, help='0: tor; 1:logged')
    ...
    #if security is logged, should have logged section on this linked profile   
    if args.security == 1:
        profile = webdriver.FirefoxProfile("/home/grad2/lspinell/.mozilla/firefox/klce1usa.novato")
